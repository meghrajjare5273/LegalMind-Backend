...
class LightweightNLPPipeline:
    ...
    def create_focused_prompt(self, risky_portions: List[Dict]) -> str:
        """Create a focused prompt with only key risky elements"""
        if not risky_portions:
            return ""
        
        # Get the highest scoring portion
        top_portion = risky_portions[0]
        sentence = top_portion['sentence']
        key_phrases = top_portion['key_phrases']
        
        # Create a condensed version focusing on key phrases
        focused_content = f"Key clause: {sentence[:200]}..."
        if key_phrases:
            focused_content += f"\nKey risk elements: {', '.join(key_phrases[:5])}"
        
        return focused_content
    
    def batch_extract_risks(self, sentences: List[str]) -> List[Dict]:
        """Batch process multiple sentences for efficiency"""
        risky_sentences = []
        
        for sentence in sentences:
            score = self._calculate_risk_score(sentence)
            if score > 0.4:  # Higher threshold for batch processing
        ...
        return sorted(risky_sentences, key=lambda x: x['score'], reverse=True)
